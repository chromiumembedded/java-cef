...
bool GeolocationHandler::OnRequestGeolocationPermission(
  ...
  JNI_CALL_METHOD(env, jhandler_, "onRequestGeolocationPermission",
                  ...
                  NewJNIString(env, requesting_url), request_id, jcallback);
...
void GeolocationHandler::OnCancelGeolocationPermission(
  ...
  if (!env)
    return;
  JNI_CALL_VOID_METHOD(env, jhandler_, "onCancelGeolocationPermission",
                       "(Lorg/cef/browser/CefBrowser;I)V",
                       GetJNIBrowser(browser), request_id);
}
